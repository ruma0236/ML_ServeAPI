...
| `EVM-241` | Dependency-aware full lifecycle profile orchestrator | In Progress | Post-W7 | exact-source run `lifecycle-20260712T144335-d8975bee` passed real VisA, GPU training, MLflow, readiness, and CI/CT; after independent approval it failed closed in deployment because the generated B0 target was `evm-bb0-staging` instead of allowlisted `evm-b0-staging`; naming-contract repair, exact-source CI, and a fresh immutable run are required while Product remains restored 1/1 |
