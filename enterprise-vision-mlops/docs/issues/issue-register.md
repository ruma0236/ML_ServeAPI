...
| `EVM-281` | Scenario B lifecycle candidate release and stable rollback | In Progress | Post-Sprint 178 | Jira `SCRUM-189`; exact-run release guard, immutable profile requirement, evidence hash closure and two-branch runner implemented. Attempt 1 stopped pre-launch on Torch-free Python. Attempt 2 reached release after real Airflow/CUDA/MLflow/CT, then failed closed because a `/mnt/evm-ct` image path was not mapped for the Windows replay host; intent remained zero and B0 was unchanged. Shared host/mount mapping and regression fixture added. Both quality and runtime branches still require fresh execution; no closure yet. |
