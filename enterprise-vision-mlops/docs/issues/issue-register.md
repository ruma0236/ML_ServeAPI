...
| `EVM-228` | Compressed W6/W7 integration review | In Progress | 2026-07-W7 | real data/model and UI evidence are complete, but final closeout now also requires EVM-226 runtime proof, EVM-236 evidence evaluator, EVM-233 environment policy, EVM-235 deployment state machine, and EVM-234 measured review-first drift event |
...
| `EVM-233` | Enterprise environment promotion policy | In Progress | 2026-07-W7 | UI/schema baseline is complete, but environment and namespace must now compute promotion eligibility; staging requires passing readiness/CI/ownership policy, while production additionally requires an allow-listed namespace, immutable digests, rollback reference, all gates, and a distinct approver |
...
| `EVM-235` | CI-gated deployment intent and state machine | In Progress | 2026-07-W7 | UI/schema baseline is complete, but CI and evidence-validator outputs must gate intent creation and the audited path must execute `dry_run -> pending_approval -> queued -> applying -> applied`, including failed and rolled-back branches |
| `EVM-236` | Enterprise evidence readiness evaluator | In Progress | 2026-07-W7 | UI checklist baseline is complete, but a content evaluator must parse and cross-check data contract, split manifest, lineage, quality gate, MLflow run, model card, artifact URI/checksum, and rollback evidence before returning reproducible `ready` or `blocked` |
