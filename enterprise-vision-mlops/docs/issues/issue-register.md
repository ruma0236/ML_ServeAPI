...
| `EVM-277` | Common lifecycle identity envelope, guard dispatcher and golden path | In Progress | Post-Sprint 178 | Jira `SCRUM-185`; common guard and single-use GPU handoff approval implemented; 433 tests at `0f1a8ab`; stale profile v8 and first cross-runtime ledger-path attempt failed closed before external dispatch; profile v9 is replay-ready and a new revision-bound L0-L7 attempt remains required |
