...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Contract Frozen / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Three pre-Q0 suites remain immutable zero-credit. Runtime-model CUDA restoration `663f783...2f3dd3` passed shared-state regression, artifact 10, broader 105, and live runtime/output `cuda:0` checks with cleanup intact and no threshold/matrix changes. Fresh artifact/Q0/calibration is required; 78-run credit remains not started. |
