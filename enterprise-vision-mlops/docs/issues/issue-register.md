...
| `EVM-279` | Scenario D idempotent lifecycle continuity and side-effect reconciliation | In Progress | Post-Sprint 178 | Jira `SCRUM-187`; fifth combined attempt `scenario-d-training-20260802T200109Z-ea1a014f` completed 10/10 stages with exact-worker detection `4.2323827 s`, recovery `7.0983775 s`, same Job continuation, MLflow, CT, approval, deploy and CUDA serving. Acceptance was 10/11 because the immediate final Prometheus snapshot caught one restart-window EOF; exact B0 UID/1/1/CUDA/plugin were restored and the same target returned up autonomously 19 s later. The attempt is immutable blocked/RCA evidence. Final restoration now requires two distinct consecutive up scrapes plus exact runtime identities within 90 s; 468/468 tests pass. A fresh source-bound PASS and hash closure remain required, so D stays open and no distributed exactly-once claim is made. |
