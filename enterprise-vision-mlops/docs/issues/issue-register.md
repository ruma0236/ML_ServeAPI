...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Contract Frozen / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Historical failures and diagnostics remain immutable zero/non-credit. Suite `x1-canonical-20260830t011424z-8e8b0bd7` passed Q0 `4/4` (`256` actual-CUDA requests) and preserved the complete first solo six-step raw bundle, then received zero credit because an undocumented zero-drain-tail safe-step condition rejected otherwise safe `25/50 RPS` measured cohorts. Independent raw reprojection selects offered `50 RPS` / service `49.9333 RPS` with accepted=terminal and zero loss, duplicate effect, or outcome unknown. Revision `bc6797938ac3dd96d0d8a319cf4d4090d47ae9c0` removes only that hidden condition; workload, measurement window, thresholds, and frozen guardrails are unchanged. A wholly fresh governed artifact/Q0/54-calibration suite is next; the 78-run credit matrix and Integrated V4 remain not started. |
