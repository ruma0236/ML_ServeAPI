...
|---|---|---|---|---|
| `EVM-EPIC-26` | S8-V4 Triton And Heterogeneous-model Operational Validation | Contract Frozen / Jira `SCRUM-209` In Progress | V3 S0-S8 verified | Coverage matrix, V3-V4 crosswalk, versioned V4 plan, append-only hash ledger, and evidence manifest committed before runtime work; E0 remains review-pending after a documentation-only conditional amendment |
| `EVM-299` | E0 Triton Environment And Profiler Baseline | Review Pending / Strict Remediation Evidence Ready / Jira `SCRUM-210` In Review | EVM-EPIC-26 | Three historical runtime repetitions remain unchanged and strictly revalidated. Evidence-bound regression suites are focused E0 `41`, mutation `26` (`1` positive + `25` negative), focused S8 closure `25`, full Python `938` with `0` skipped, real PostgreSQL `40`, lifecycle/host `144`, Control Panel Python/UI `77/59`, status/evidence `105`, and frontend build `1,803` modules. The prior external shorthand `focused 44` is superseded as unbound; reviewer sign-off and verified credit remain pending. |
