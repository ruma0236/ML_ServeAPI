...
| `S7` | `EVM-297` | Image/VLM/LLM Auxiliary Admission | Implementing | Post-W8 | Jira `SCRUM-207`. Existing image and generative serving paths enforce a versioned family-cost admission contract. An external-HTTP real-CUDA runner uses the existing exact GPU lease, B0 handoff/restore boundary, Prometheus file discovery, process-tree resource sampling, and raw-derived evidence validator. Two rejected diagnostic checkpoints retain their zero-credit RCA. At the strict cleanup revision, fresh image/VLM/LLM diagnostics completed 18/18 external-HTTP CUDA requests and restored B0, lease, S7 target, and Prometheus 5/5 exactly; this closes readiness only. The 12-profile × 3-repetition matrix remains uncredited and `S7-AC-01..04` are pending. |
