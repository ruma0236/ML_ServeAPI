...
| `S4` | `EVM-294` | HIGGS Tiny MLP GPU Batching | Implementing | Post-W8 | no-catch-up pacing now measures delivery fidelity; 107.4 RPS failed while three 80 RPS calibration repeats passed fixed p99/queue SLO, trace, OOM, drain and cleanup gates. The 80 RPS ceiling and validated-service-rate S2 queue formula are implemented; fresh full closure remains pending |
