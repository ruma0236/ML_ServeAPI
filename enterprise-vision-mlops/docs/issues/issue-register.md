...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | The frozen model/topology contract remains unchanged. First fresh suite `x1-canonical-20260829t195712z-1722de47` failed before Q0 on a reproducible Torch-after-PyArrow Windows DLL initialization conflict; its cleanup and private evidence are immutable zero-credit. Minimal import-order remediation and a completely fresh artifact/Q0/calibration run are required; the 78-run matrix remains not started. |
