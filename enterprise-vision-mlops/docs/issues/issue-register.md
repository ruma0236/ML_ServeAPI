...
| `S4` | `EVM-294` | HIGGS Tiny MLP GPU Batching | Implementing | Post-W8 | no-catch-up pacing and 60 RPS calibration pass, but the first quiet gate falsely attributed Windows display-GPU spikes to the experiment after 63 valid repetitions. Exact lease/container ownership, stable VRAM/temperature, and a baseline-relative terminal median now define the 60-second recovery gate; external pilot and fresh full closure remain pending |
