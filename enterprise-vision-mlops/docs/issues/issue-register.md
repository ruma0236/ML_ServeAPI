...
| `EVM-241` | Dependency-aware full lifecycle profile orchestrator | In Progress | Post-W7 | run `lifecycle-20260712T144335-d8975bee` exposed invalid `evm-bb0-staging` after approval; commit `9de5600` fixed naming and run `lifecycle-20260712T151704-45416b2b` re-passed real VisA, GPU training, MLflow, 13 in-memory readiness checks, and CI/CT, but was cancelled before approval because its referenced `readiness.json` was not persisted; atomic readiness-report persistence, exact-source CI, and one fresh immutable run remain while Product is restored 1/1 |
