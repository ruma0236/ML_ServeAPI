...
| `EVM-266` | Scenario A GPU and serving failure recovery | In Progress | Sprint 178 | A0-A7 implementation, 49 focused tests, real production-candidate CUDA CT, read-only target baseline, and no-mutation device-plugin planning are implemented; run `scenario-a-20260801T161231Z-534f4ac2` was correctly blocked by a malformed operator path and retained for RCA; a fresh A5-A7 run and three approved exact-Pod A8 repetitions remain |
