...
| `EVM-300` | S6B-M Triton Model Blue/Green | Remediation Required / Zero-Credit / Jira `SCRUM-211` In Review | EVM-299 verified | Strict-v3 review failed on a coherent cross-clock span-start mutation and the absence of a canonical durable terminal effect. Strict-v4 freezes request-independent dual-clock anchors plus a real PostgreSQL commit/readback effect boundary and requires a fresh `3 + 3 + 15` matrix. V1-V3 history is immutable; X1 remains blocked. |
