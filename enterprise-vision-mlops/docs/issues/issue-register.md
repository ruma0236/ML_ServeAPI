...
| `EVM-279` | Scenario D idempotent lifecycle continuity and side-effect reconciliation | In Progress | Post-Sprint 178 | Jira `SCRUM-187`; exact Job reconciliation and ledger guards pass non-disruptive replay `scenario-d-lifecycle-20260802T182104Z-fdcf0047` with 18 decisions, zero external identity delta and 62/62 hashes. Exact worker/observer/worker live series has detection max 5.212014 s, recovery max 8.6442343 s and 24/24 hashes. First combined run exposed missing GPU handoff approval before mutation. Second run `lifecycle-20260802T190057-c8cae6d4` passed Airflow 18/18 and admitted the real training Job, then blocked before worker kill because runner/reconciler fixtures used a stale run-label formula. Automatic cancel released the GPU handoff in 48 s and restored B0 CUDA. All paths now share canonical `short_run_id()` and exact manifest identity; 464/464 tests pass. A fresh combined attempt remains required, so D stays open and no distributed exactly-once claim is made. |
