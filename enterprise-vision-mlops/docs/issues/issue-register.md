...
|---|---|---|---|---|
| `EVM-EPIC-26` | S8-V4 Triton And Heterogeneous-model Operational Validation | Contract Frozen / Jira `SCRUM-209` In Progress | V3 S0-S8 verified | Coverage matrix, V3-V4 crosswalk, versioned V4 plan, append-only hash ledger, and evidence manifest committed before runtime work; E0 is verified while S6B-M and later work retain independent gates |
...
| `EVM-300` | S6B-M Triton Model Blue/Green | Remediation Running / Non-Credit / Jira `SCRUM-211` In Review | EVM-299 verified | Historical handoffs remain immutable and non-credit. Remediation v2 was rejected because hold/drain ordering could be inferred from summary fields without proving the raw request/span/effect/pre-unload timeline. Strict v3 reprojects the unchanged fresh `3 + 3 + 15` runtime suite, records the independently unverifiable historical partial count as an append-only amendment, and requires `28/28` fail-closed mutations before returning to review. X1 remains blocked. |
