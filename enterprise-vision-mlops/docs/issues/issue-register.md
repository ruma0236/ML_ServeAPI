...
| `S3` | `EVM-291` | HIGGS Lightweight Capacity Envelope | Implementing | Post-W8 | Jira `SCRUM-201`. The existing Workloads API exposes a versioned tabular capacity catalog and prediction route for the frozen five families. Dataset, split, model artifact, and request identities fail closed; validation/transform/prediction spans and bounded metrics are wired. At `bf76c44`, the governed source preparation streamed all `11,000,000` rows, preserved the official split boundary, produced deterministic bounded samples and five immutable numeric model artifacts, matched all `12` split and `5` model hashes, and left no temporary build directory. External Uvicorn TCP smoke returned `200` for all five predictions and the existing VLM/LLM preset contract, exposed four S3 metric families, and cleaned up. Preparation and smoke evidence are recorded at `docs/status/evidence/s3-higgs-preparation-checkpoint.json`; the full load matrix has not run and all S3 ACs remain pending. |
