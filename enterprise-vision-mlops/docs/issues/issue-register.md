...
| `S4` | `EVM-294` | HIGGS Tiny MLP GPU Batching | Implementing | Post-W8 | batch-one preparation passed; two fail-closed attempts isolated exporter timing and per-request trace ownership, with cleanup intact; the corrected 3-run trace pilot, fresh 60+3+3 matrix and all ACs remain pending |
