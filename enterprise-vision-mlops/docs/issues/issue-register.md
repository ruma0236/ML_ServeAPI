...
| `S7` | `EVM-297` | Image/VLM/LLM Auxiliary Admission | Implementing | Post-W8 | Jira `SCRUM-207`. Existing image and generative serving paths enforce a versioned family-cost admission contract. An external-HTTP real-CUDA runner uses the existing exact GPU lease, B0 handoff/restore boundary, Prometheus file discovery, process-tree resource sampling, and raw-derived evidence validator. The first image warmup was rejected with zero credit after host input was remapped to a container-only mount; exact lease/process/B0 CUDA/Prometheus/target cleanup passed, and host-root plus file-SD convergence remediation is implemented. Full sequential CUDA profiles remain uncredited; `S7-AC-01..04` are pending. |
