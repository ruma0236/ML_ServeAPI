...
| `EVM-283` | Integrated single-scenario lifecycle evidence closure | In Progress | Post-Sprint 178 | Jira `SCRUM-191`; validate that every A-E guard reaches its real lifecycle injection point, has deterministic decision evidence, full hash closure and immutable RCA; blocks EVM-274 until PASS |
...
Cross-scenario correlation remains blocked on EVM-283's combined evidence and
lifecycle-reachability validation. C's
