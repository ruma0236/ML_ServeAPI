...
| `S8` | `EVM-298` | Dependency Soak & Resource-efficiency Closure | Implementing | Post-W8 | external runner and independent raw/Git-blob validator are implemented. Attempt 01 retained a pre-admission Prometheus host-port TOCTOU race; attempt 02 passed nine fresh control/latency/transient repetitions but exposed runner/config drift and five expiry outcomes in retry-budget repetition 1. Both attempts have zero acceptance credit and complete cleanup. The v2 profile reads the declared 12 transient + 4 healthy workload, freezes an 8-per-60-second global budget below circuit throughput, and records monotonic fault-to-terminal elapsed time. A fresh 21-repetition matrix, 30-minute x3 soak, regressions and hash closure remain pending. |
...
its failed attempts and rollback boundary remain independent operational evidence. EVM-295 remains verified with the narrowed drain-evidence claim above. EVM-296 and EVM-297 are verified after strict reclosure, while EVM-298 remains planning-only and was not started in this audit turn.
