...
| `S3` | `EVM-291` | HIGGS Lightweight Capacity Envelope | Implementing | Post-W8 | Jira `SCRUM-201`. The existing Workloads API exposes a versioned tabular capacity catalog and prediction route for the frozen five families. Dataset, split, model artifact, and request identities fail closed; validation/transform/prediction spans and bounded metrics are wired. At `bf76c44`, the governed source preparation streamed all `11,000,000` rows, produced deterministic samples and five immutable model artifacts, matched all `12` split and `5` model hashes, and left no temporary build directory. Revision `69876df` adds process-local count/byte admission, fixed CPU workers, bounded wait/timeout, `413`/`429 + Retry-After`, queue/in-flight telemetry, cross-thread traces, and lifespan cleanup. Its clean external pressure smoke produced `32×200`, `68×429`, `100/100` trace IDs, and terminal queue/byte/in-flight/outstanding gauges all zero. Public checkpoints are `s3-higgs-preparation-checkpoint.json` and `s3-bounded-executor-checkpoint.json`; the full three-repeat load matrix has not run and all S3 ACs remain pending. |
...
EVM-290, EVM-292, and EVM-293 are verified from canonical evidence. EVM-291
and EVM-294..298 remain planning-only.
