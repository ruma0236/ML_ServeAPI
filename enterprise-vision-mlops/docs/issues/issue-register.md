...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Contract Frozen / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Historical failures and diagnostics remain immutable zero/non-credit. Suite `x1-canonical-20260830t022408z-ea5b3f5f` passed Q0 `4/4` (`256` actual-CUDA requests) and two solo repetitions, then received zero credit when rep3's `400 RPS` warmup had one host `ConnectionError` / outcome-unknown among `4,000` offers; `1,557` completed requests match `1,557` durable effects and `2,442` offers were bounded local rejections. Private inventory is `47` files / `331,676,544` bytes, aggregate `684b5c28...f8821`, index `29d81dfe...f163f`. Revision `5eda828989c9005dd3c28aa439e0583a31f5793d` adds bounded future transport diagnostics only; retry, timeout, workload, thresholds, Q0, and matrices remain frozen. A wholly fresh governed artifact/Q0/54-calibration suite is next; the 78-run credit matrix and Integrated V4 remain not started. |
