...
| `EVM-285` | Full lifecycle guard validation execution ledger | In Progress | Post-Sprint 178 | Jira `SCRUM-193` under Epic `SCRUM-183`; replacement suite `full-lifecycle-actual-injection-20260803T062614Z-d83a51f0` runs `E -> C -> B -> D -> A`, one fresh run/injection at a time. E/C/B are PASS. D attempt `lifecycle-20260805T091303-98a14793` at source `2d5f056` stopped exact worker PID 54376 only after Job/task/key binding, restored one PID 68368 on the same revision/lease/fence, reconciled the same Job without redispatch, completed 10/10 lifecycle, eight unique effects and exact B0 runtime restoration in 18.250 s. It remains immutable BLOCKED/no-credit because detection `11.032 s` exceeded fixed `<=10 s` although recovery `13.579 s` passed. Result SHA `05fd6f88...ee8fd`. Remediation `6e2ec01` preserves the SLO and aligns polling at 3 s. Retry admission run `lifecycle-20260805T094807-749cbdd5` failed closed before injection because stale API source `2d5f056` sealed the run while exact children were `e02bdd7`; effects were zero. Code `ed29285` exposes API source and requires exact API/supervisor/worker/observer revision before run creation; stale proof kept run count `74 -> 74`. First deployment exposed a `GIT_*` versus `EVM_GIT_*` readiness alias mismatch with source null and no run/mutation; `8167608` aligns lifecycle precedence and passes 14 focused, 528 full and Ruff. Recreate only API with `--no-deps`, converge children, and retry D fresh. A remains unstarted. |
