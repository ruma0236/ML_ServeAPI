...
| `EVM-EPIC-25` | Distributed Scale And Operational Load Validation | Post-W8 | In Progress | S0-S7 are verified after strict post-closure audit; S8 dependency-fault and soak/resource-efficiency work is implementing with five rejected zero-credit attempts retained. Single-node limits remain explicit. |
...
| `S8` | `EVM-298` | Dependency Soak & Resource-efficiency Closure | Implementing | Post-W8 | external runner and independent raw/Git-blob validator are implemented. Attempts 01-03 retain, respectively, a Prometheus host-port TOCTOU race, retry-budget runner/config drift, and an equal 10-second worker/HTTP timeout ambiguity. A zero-credit v2 calibration closed 17/17 tasks and the v5 timeout preflight later passed 6/6 terminal/trace closure with 51.484-second recovery. Attempt 04 isolated worker-loss MTTR projection from a sequential timeout/HOL phase; V6's zero-credit worker preflight measured 11.954 seconds versus 62.234 seconds full profile elapsed. Attempt 05 passed all 21 V6 fault scopes, then invalidated its first 30-minute soak because a transient one-second `/metrics` read timeout terminated the resource sampler. The first sampler preflight completed 4,200/4,200 requests at 35 RPS with zero errors and p99 40.95 ms but produced only 55 of 108 required gauge samples because direct metrics reads competed with the API. All failed attempts and preflights have zero acceptance credit and cleanup evidence. Runtime gauges now use complete, finite, fresh timestamped samples from the existing isolated Prometheus cache; a second non-credit validation, then a fresh 21-repetition matrix and 30-minute x3 soak remain pending. |

The canonical planning contract is
`docs/agenda/2026-08-15-distributed-scale-operational-validation-plan-v3.md`.
EVM-290 through EVM-297 are verified from canonical evidence. EVM-296 and EVM-297 strict reclosures preserve their historical accepted matrices and superseded v1 closures; no new matrix acceptance credit was created by evidence reprojection.
...
its failed attempts and rollback boundary remain independent operational evidence. EVM-295 remains verified with the narrowed drain-evidence claim above. EVM-296 and EVM-297 are verified after strict reclosure. EVM-298 is implementing: five rejected full attempts, one zero-credit retry calibration, one rejected v4 timeout preflight, and passing zero-credit v5 timeout and v6 worker-MTTR preflights are retained. No acceptance repetition or soak result is credited before a fresh post-remediation run.
