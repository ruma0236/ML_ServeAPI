...
| `EVM-279` | Scenario D idempotent lifecycle continuity and side-effect reconciliation | In Progress | Post-Sprint 178 | Jira `SCRUM-187`; exact replay and child-live proofs remain accepted. Fourth combined attempt `scenario-d-training-20260802T193451Z-b82f6b49` reached the real active-training boundary: exact worker detection `2.3490377 s`, recovery `5.631701 s`, same Job UID continuation, MLflow, readiness 13/13 and isolated CT 2,181 all passed. Release approval then failed closed because the API container could not map the F-drive CT URI to `/mnt/evm-ct`; automatic cancellation restored B0 1/1 CUDA with zero approval/intent/candidate-serving mutation. CT host/mount resolution is fixed and 466/466 tests pass. Container contract proof and a fresh autonomous 10/10 lifecycle remain required, so D stays open and no distributed exactly-once claim is made. |
