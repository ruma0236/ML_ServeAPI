...
| `EVM-300` | S6B-M Triton Model Blue/Green | Remediation Required / Jira `SCRUM-211` In Progress | EVM-299 verified | Five failed suites remain immutable and zero-credit. The latest completed the runtime matrix and cleanup but failed public Git-blob projection because the config path omitted the parent Git-root prefix (SHA `fb62d3f7...c4414a`). Source binding now resolves and validates before mutation; all AC remain pending until a fresh full rerun. |
