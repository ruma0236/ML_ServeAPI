...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Ten failed suites remain immutable zero-credit. Latest suite `x1-canonical-20260829t221816z-2d6e5108` completed 193 direct Triton inferences before a bounded NodePort read timeout; no Q0 bundle or calibration received credit, and exact cleanup passed. A bounded no-proxy Q0 session remediation and wholly fresh artifact/Q0/54-calibration suite remain required; 78-run credit remains not started. |
