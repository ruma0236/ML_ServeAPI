...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Twelve failed suites remain immutable zero-credit. Non-credit diagnostic `x1-diagnostic-20260829t233121z-e70de403` reproduced one-success/second-request-timeout with Triton tracing both on and off; tracing is not the differentiator. The pinned Triton image lacks Python torch, so direct-image replay was unavailable rather than an artifact failure. Host-PyTorch and DLRM-only Triton isolation is next; Q0/54 calibrations and 78-run credit remain not started. |
