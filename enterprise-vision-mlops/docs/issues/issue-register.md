...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Contract Frozen / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Historical failures and diagnostics remain immutable zero/non-credit. Revision `f1e34425b1de949dbe4b25d70349587ae700d5dd` freezes a field-offset DLRM embedding artifact plus an exact 64-call/30-second saved-TorchScript CUDA repeatability gate. A wholly fresh governed artifact/Q0/54-calibration suite is next; the 78-run credit matrix and Integrated V4 remain not started. |
