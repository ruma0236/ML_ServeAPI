...
| `S8` | `EVM-298` | Dependency Soak & Resource-efficiency Closure | Implementing | Post-W8 | external runner and independent raw/Git-blob validator are implemented. Attempts 01-03 retain, respectively, a Prometheus host-port TOCTOU race, retry-budget runner/config drift, and an equal 10-second worker/HTTP timeout ambiguity. A zero-credit v2 calibration closed 17/17 tasks and the v5 timeout preflight later passed 6/6 terminal/trace closure with 51.484-second recovery. Attempt 04 then passed all 21 individual fault scopes with duplicate effects and pool timeouts zero, but aggregate admission rejected the run because the worker-loss MTTR projection included a sequential timeout/HOL phase and measured 61.608-63.234 seconds against the unchanged 60-second bound. All failed attempts have zero acceptance credit and complete cleanup. V6 separates exact PID-failure-to-reclaimed-task worker MTTR from full profile elapsed. A fresh 21-repetition matrix, 30-minute x3 soak, regressions and hash closure remain pending. |
