...
| `EVM-226` | Docker Desktop Kubernetes B7 training and serving proof | In Progress | 2026-07-W7 | Docker Desktop Kubernetes is the fixed W7 cluster; this issue absorbs EVM-227 execution scope and requires the selected B7 training Job plus serving Deployment with `nvidia.com/gpu` request/limit, namespaces, probes, logs, artifact checksums, inference, controlled failure, and rollback evidence; currently blocked because Kubernetes is disabled and no node advertises GPU |
