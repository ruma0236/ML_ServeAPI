...
| `S4` | `EVM-294` | HIGGS Tiny MLP GPU Batching | Implementing | Post-W8 | the first complete 60+3+3 run retained 208,058 requests, 449/449 sampled traces, zero OOMs, hard safety and exact cleanup, but the selected point at 80% saturation passed the fixed operating SLO only once; 30% headroom is frozen for a fresh full rerun and all ACs remain pending |
...
EVM-294 is implementing from hash-linked preparation and five retained RCA checkpoints;
its first full 60+3+3 run failed reproducible operating latency, and a fresh full rerun
with the thresholds unchanged remains pending. EVM-295..298
