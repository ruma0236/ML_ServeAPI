...
| `S7` | `EVM-297` | Image/VLM/LLM Auxiliary Admission | Verified | Post-W8 | Jira `SCRUM-207`. A clean 36-repetition external-HTTP real-CUDA matrix completed 12 repetitions per family, 162 admitted requests, and 54 intentional bounded rejections with zero OOM/starvation. Independent raw-derived validation closed distinct family p95/p99 and quality schemas, fairness/HOL, trace/Prometheus attribution, and unsupported-metric absence. Current-revision CUDA smoke completed `6/6`; `54` focused, `38` real-PostgreSQL, `835` full Python, `147` lifecycle/host, `77` Control Panel Python, and `59` frontend tests plus the production build passed. B0 returned `1/1`, queues/leases and S7 targets returned `0`, and Prometheus returned `5/5`. Three failed attempts remain with zero credit. Local single-node/single-GPU controlled evidence only. |
...
EVM-290, EVM-291, EVM-292, EVM-293, EVM-294, EVM-295, EVM-296, and EVM-297 are verified from canonical evidence.
...
verified by the strict S5 experiment and closure artifacts; EVM-295 is verified by the strict S6 experiment and closure artifacts. EVM-297 is verified by the strict S7 experiment and closure artifacts, while EVM-298 remains planning-only.
