...
|---|---|---|---|---|
| `EVM-286` | Generic scenario workload lifecycle contract and RTX 4080 executor | In Progress / contract and data-view core implemented | Post-Sprint 178 | Jira `SCRUM-195` under Epic `SCRUM-194`; exact identity/state/fenced GPU lease/final evidence seal plus immutable ScienceQA/Dolly derived-view preparation are implemented. Runtime pins import together and a real SmolVLM CUDA preflight passed. Focused tests 8/8 plus prior scenario/control regressions and Ruff pass. API/worker and accepted real lifecycle remain open; no model run is credited yet. |
