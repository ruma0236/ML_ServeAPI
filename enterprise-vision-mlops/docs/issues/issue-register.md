...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Five pre-Q0 suites remain immutable zero-credit. The fifth confirmed exact WSL mounts and zero-error cleanup but exposed Triton CUDA compatibility libraries preceding mounted WSL driver shims. Exact library precedence remediation/refreeze and a wholly fresh artifact/Q0/calibration suite are required; 78-run credit remains not started. |
