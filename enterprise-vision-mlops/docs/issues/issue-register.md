...
| `S6` | `EVM-295` | API Rolling Continuity & GPU Controlled Handoff | Implementing | Post-W8 | Jira `SCRUM-205`. The existing FastAPI now has drain-aware admission and a PostgreSQL-idempotent terminal rollout probe; the API Deployment declares two replicas, zero-unavailable RollingUpdate, startup/readiness/liveness probes, preStop drain, and bounded termination grace. An isolated S6 Deployment preserves the Compose API and B0 serving baseline. Focused unit/API/real-PostgreSQL contract tests pass, but API rolling and single-GPU handoff acceptance repetitions have not started; all ACs remain pending. Local single-node controlled scope only. |
...
EVM-290, EVM-291, EVM-292, EVM-293, EVM-294, and EVM-296 are verified from canonical evidence.
...
verified by the strict S5 experiment and closure artifacts; EVM-295 is implementation-only with no acceptance credit, while EVM-297 and EVM-298 remain planning-only.
