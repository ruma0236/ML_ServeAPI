...
|---|---|---|---|---|
| `EVM-EPIC-26` | S8-V4 Triton And Heterogeneous-model Operational Validation | Contract Frozen / Jira `SCRUM-209` In Progress | V3 S0-S8 verified | E0 and S6B-M Strict V4 are verified. Canonical X1 is contract-frozen/non-credit with fresh artifact/Q0/capacity calibration next; its 78-run credit matrix and Integrated V4 remain not run. |
...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Suite `x1-canonical-20260830t075925z-7a121261` remains immutable zero-credit. RCA v2 confirms three synchronous entity/idempotency commits with separate-transaction visibility were incorrectly converted to HTTP `503` by a generic X1 wall-clock ordering veto; implementation `43054593091a27f6054982e5823422983de6ee52` removes that unintended causal dependency while preserving commit ambiguity fail-closed behavior and the separate S6B-M strict clock lane. Three independent `120 s` / `1,200`-sample diagnostic-v2 windows observed Windows steps `0/0/0`, WSL `8/8/8`, Docker `8/8/8`, and PostgreSQL `8/6/8`; the underlying synchronization mechanism remains unknown. Public RCA `1a2e3116...59ec0`, private diagnostic inventory `10` files / `3,679,118` bytes, aggregate `dedb57ae...40ac1`, index `14f26a82...680d8`. Exact B0 actual-CUDA, GPU/device-plugin `1/1`, Prometheus `5/5`, queues `0/0/0`, GPU lease/runtime/schema residue `0` are restored. Fresh Q0/calibration remains NO-GO; the 78-run matrix and Integrated V4 are not started, and WSL time remediation is a separate next task. |
