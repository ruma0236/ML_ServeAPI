...
| `EVM-300` | S6B-M Triton Model Blue/Green | Running / Jira `SCRUM-211` In Progress | EVM-299 verified | Six failed suites remain immutable and zero-credit. Remediation `b908d5a` adds bounded per-worker persistent sessions, primary transport diagnostics and post-failure cleanup evidence; 23 focused tests passed. The complete matrix restarted from that clean revision and all AC remain pending/non-credit. |
