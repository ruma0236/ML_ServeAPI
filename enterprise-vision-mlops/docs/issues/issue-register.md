...
| `EVM-281` | Scenario B lifecycle candidate release and stable rollback | In Progress | Post-Sprint 178 | Jira `SCRUM-189`; exact-run guard and two-branch runner implemented. Attempt 1 fixed Torch runtime discovery. Attempt 2 fixed CT host/mount replay paths after a full real lifecycle. Attempt 3 passed the 1,000-record loader but caught one post-CT Prometheus restart-window scrape; replay/guard/approval did not execute, intent stayed zero and exact B0 recovered on a distinct scrape about 22 s later. Pre-replay admission now requires exact runtime identity plus two distinct consecutive up scrapes. Both branches still require fresh execution; no closure yet. |
