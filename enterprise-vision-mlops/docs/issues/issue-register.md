...
| `EVM-282` | Scenario A post-promotion GPU serving recovery | In Progress / first live attempt failed / M0 recovered / remediation pending fresh proof | Post-Sprint 178 | Jira `SCRUM-190`; attempt `scenario-a-lifecycle-20260803T000920Z-0209bac1-47eaa66e` verified M1 then failed on a 275-character Windows atomic state path. Exact delayed rollback `rollback-recovery-20260803T001831Z` restored M0/CUDA/two Prometheus scrapes/pointer without acceptance credit. Compact path preflight and exact Recreate reconciliation are implemented for a fresh immutable attempt. |
...
recovery; cross-scenario correlation remains blocked on a fresh A PASS. C's candidate remains on manual hold
