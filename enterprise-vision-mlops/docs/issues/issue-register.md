...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Eleven failed suites remain immutable zero-credit. Failure-time NodePort versus port-forward diagnostics are frozen at `31e91222...15c0286`; focused `12` and broader `136` pass without timeout, retry, cardinality, workload, threshold, topology, batching, or matrix change. One fresh zero-credit diagnostic suite must classify the fault before any Q0/54-calibration remediation; 78-run credit remains not started. |
