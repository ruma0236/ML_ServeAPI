...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Historical suite `x1-canonical-20260830t075925z-7a121261`, RCA v2, and ledger through `s8-v4-0269` remain immutable. Remediation run `x1-clock-remediation-20260830T113944Z-34eec036` used the unchanged `180 s`/`100 ms` gate and failed its first Docker-off window: Windows discontinuities `0`, WSL `5`, backward `0`, unclassified gaps `0`, with four exact `34.8 s` intervals. No service event correlated with the steps, so guest time-service and Hyper-V implicit-sync settings were unchanged. `wsl --update` timed out after 180 seconds without output, `wsl --shutdown` passed, and official WSL ETW was not run because the session was non-admin. Public RCA `ca0bc72d...b66aa`; private inventory `17` files / `5,820,112` bytes, aggregate `8548570b...a777a`, index `b5b98f58...37a4f`. Docker/compose and PostgreSQL were restored; B0 actual-CUDA and GPU `1/1`, Prometheus `5/5`, queues `0/0/0`, and temporary runtime/lease/schema residue `0` pass. Decision remains `x1_clock_remediation_no_go`; full-stack windows, Q0, calibration, the 78-run matrix, and Integrated V4 were not started. |
