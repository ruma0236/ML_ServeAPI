...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Nine failed suites remain immutable zero-credit. Fresh suite `x1-canonical-20260829t220949z-5a1ba909` entered Q0 and failed closed because batch-disabled GaussianNB and DLRM-lite TorchScript artifacts assumed a two-dimensional input; persisted-artifact replay reproduced the defect. Shape-safe artifacts, clean refreeze, and a wholly fresh artifact/Q0/54-calibration suite remain required; 78-run credit remains not started. |
