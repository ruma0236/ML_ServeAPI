...
| `S8` | `EVM-298` | Dependency Soak & Resource-efficiency Closure | Implementing | Post-W8 | external runner and independent raw/Git-blob validator are implemented. Attempt 01 retained a pre-admission Prometheus host-port TOCTOU race; attempt 02 exposed retry-budget runner/config drift; attempt 03 passed 15 fresh fault repetitions before two timeout requests remained outcome-unknown at an equal 10-second worker/HTTP timeout boundary. All three attempts have zero acceptance credit and complete cleanup. A separate zero-credit v2 calibration closed 17/17 tasks with duplicate effects zero and measured 40.109 seconds to terminal recovery. The v4 profile keeps the declared 12 transient + 4 healthy workload, 8-per-60-second retry budget and 60-second MTTR guardrail, while timeout work must fail closed with zero effect after a bounded 30-second exact-effect observation window and healthy work must complete. A fresh 21-repetition matrix, 30-minute x3 soak, regressions and hash closure remain pending. |
