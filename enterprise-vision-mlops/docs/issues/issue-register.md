...
| `EVM-269` | Scenario D lifecycle supervision recovery | In Progress | Sprint 178 | D0 contract is `docs/status/2026-08-02-scenario-d-lifecycle-supervision-contract.md`: exact PID/start/command/revision/lease/fence identity, state machine, duplicate/unknown fail-closed policy, persistent restart budget/backoff/circuit breaker, fenced run claim, API/Prometheus signals, deterministic fixture matrix, and three exact-child live runs. A/P0 evidence is baseline-only; implementation and proof remain pending. |
