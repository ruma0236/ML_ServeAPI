...
| `EVM-300` | S6B-M Triton Model Blue/Green | Running / Jira `SCRUM-211` In Progress | EVM-299 verified | Four failed suites remain immutable and zero-credit, including cleanup-race SHA `dcd35d18...fdcf2`. Remediation `8dc732f` waits for exact Prometheus baseline equality and passed 20 focused tests; the complete controlled matrix restarted from that clean revision. All AC remain pending/non-credit. |
