...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Suite `x1-canonical-20260830t075925z-7a121261` is immutable zero-credit after Q0 `4/4`, LogisticRegression `3/3`, GaussianNB `3/3`, and a TinyMLP rep1 `200 RPS` step with three post-effect-commit HTTP `503` failures caused by a detected PostgreSQL wall-clock regression. Private inventory is `55` files / `1,111,494,392` bytes, aggregate `82e152b2...41aa`, index `d4445ad8...5b38`; cleanup is exact. One bounded Docker Desktop restart restored exact B0/CUDA, GPU `1/1`, Prometheus `5/5`, queues `0/0/0`, and zero X1 runtime/lease residue, but a new 1,200-sample probe again observed six forward discontinuities over `100 ms`, including three over one second and a maximum `2,046,914,650 ns` residual. Fresh runtime remains forbidden. Contract, thresholds, six-step schedule, 54-run calibration, 78-run credit matrix, and Integrated V4 scope are unchanged; the latter two remain not started. |
