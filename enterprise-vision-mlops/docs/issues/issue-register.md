...
| `S4` | `EVM-294` | HIGGS Tiny MLP GPU Batching | Implementing | Post-W8 | current-revision batch-one preparation smoke passed through the existing Workloads API with CUDA, Prometheus/OTLP, terminal drain and exact holder restoration; 60+3+3 matrix and all ACs remain pending |
