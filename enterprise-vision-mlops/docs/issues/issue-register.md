...
| `S2` | `EVM-293` | Bounded Queue & Backpressure | Implementing | Post-W8 | hardening checkpoint connects atomic PostgreSQL admission, canonical item bytes, distinct 413/429 + Retry-After, dedicated bounded workers, owner/epoch fencing, retry/DLQ, lock-free external dispatch relative to the legacy ledger, row-level task state, and bounded terminal-history rollups; 58 focused tests passed, while external throughput, process-tree memory, three-run overload/restart/GPU/full-regression evidence and all S2 ACs remain pending |
