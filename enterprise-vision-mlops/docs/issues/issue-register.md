...
| `S8` | `EVM-298` | Dependency Soak & Resource-efficiency Closure | Implementing | Post-W8 | external runner and independent raw/Git-blob validator are implemented. Attempts 01-03 retain, respectively, a Prometheus host-port TOCTOU race, retry-budget runner/config drift, and an equal 10-second worker/HTTP timeout ambiguity; all have zero acceptance credit and complete cleanup. A zero-credit v2 calibration closed 17/17 tasks with duplicate effects zero and measured 40.109 seconds to terminal recovery. The v4 timeout preflight reached 6/6 terminal outcomes but failed trace 2/6 and MTTR 72.828 seconds because injected delay also affected healthy work. The zero-credit v5 preflight then passed 6/6 terminal and trace closure, exact 4/4 eligible effects, zero duplicate/multiple effects, two explicit no-effect timeout failures, mirror parity, 51.484-second recovery, and cleanup. Acceptance credit remains zero; a fresh 21-repetition matrix, 30-minute x3 soak, regressions and hash closure remain pending. |
...
its failed attempts and rollback boundary remain independent operational evidence. EVM-295 remains verified with the narrowed drain-evidence claim above. EVM-296 and EVM-297 are verified after strict reclosure. EVM-298 is implementing: three rejected full attempts, one zero-credit retry calibration, one rejected v4 timeout preflight, and one passing zero-credit v5 timeout preflight are retained. No acceptance repetition or soak result is credited before a fresh v5 run.
