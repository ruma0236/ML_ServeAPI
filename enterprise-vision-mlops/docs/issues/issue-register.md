...
| `S3` | `EVM-291` | HIGGS Lightweight Capacity Envelope | Implementing | Post-W8 | Jira `SCRUM-201`. The existing Workloads API now exposes a versioned tabular capacity catalog and prediction route for the frozen logistic, probabilistic, online-linear, branch-heavy, and incremental probe families. Dataset, split, model artifact, and request identities fail closed; validation/transform/prediction spans and bounded metrics are wired. Focused API/compatibility/metrics/ledger verification passed `45` tests plus Ruff and the canonical progress validator. No HIGGS source intake, trained probe artifacts, external load sweep, capacity result, or S3 acceptance credit exists yet. |
