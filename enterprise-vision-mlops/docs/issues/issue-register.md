...
| `EVM-300` | S6B-M Triton Model Blue/Green | Remediation Required / Jira `SCRUM-211` In Progress | EVM-299 verified | Six failed suites remain immutable and zero-credit. The latest suite encountered one pre-API Windows transport failure in repetition 3 (SHA `14d26d34...3b15a9`) and was rejected; cleanup restored B0 CUDA, Prometheus 5/5 and zero queue/lease/runtime residue. Bounded per-worker persistent sessions and primary transport diagnostics are under fresh regression; all AC remain pending. |
