...
| `EVM-277` | Common lifecycle identity envelope, guard dispatcher and golden path | In Progress | Post-Sprint 178 | Jira `SCRUM-185`; common guard and single-use GPU handoff approval implemented; 434 tests at `1d39845`; stale profile v8 plus two independent cross-runtime evidence-path attempts failed closed before external dispatch; profile v9 is replay-ready, both path RCAs are fixed, and a new revision-bound L0-L7 attempt remains required |
