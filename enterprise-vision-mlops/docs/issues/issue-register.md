...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Twelve failed suites and diagnostic `x1-diagnostic-20260829t233121z-e70de403` remain immutable zero/non-credit. Trace-enabled and trace-disabled Triton both reproduced one-success/second-request-timeout; host-PyTorch CUDA plus DLRM-only Triton follow-up is frozen at `54b48f87904d3e0c1504fc2ad8a8db050701bead`. Q0/54 calibrations and 78-run credit remain not started. |
