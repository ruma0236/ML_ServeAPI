...
| `EVM-279` | Scenario D idempotent lifecycle continuity and side-effect reconciliation | In Progress | Post-Sprint 178 | Jira `SCRUM-187`; integrated review confirmed the post-admission `reserved` ledger gap. Exact read-only Kubernetes Job identity reconciliation, ledger uniqueness/state guards, degraded-supervisor fail-closed admission, and a six-branch three-replay runner are implemented with 50/50 focused and 458/458 full tests. F-drive execution and bounded exact worker/observer live proof remain open; no distributed exactly-once claim. |
