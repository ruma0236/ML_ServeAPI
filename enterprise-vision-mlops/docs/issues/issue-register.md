...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Contract Frozen / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Historical failures and diagnostics remain immutable zero/non-credit. Suite `x1-canonical-20260830t033533z-ec3ec195` passed Q0 `4/4` (`256` actual-CUDA requests), solo `12/12`, and topology `r1/w1` `3/3`, then received zero credit when the first `r1/w2` `25 RPS` warmup produced `248/250` completed/effects, one HTTP `503`, and one `RemoteDisconnected` outcome-unknown request. Private inventory is `74` files / `2,357,132,021` bytes, aggregate `85181f0c...958ae`, index `b2582dab...35bdb`. Revision `715ebe8a91615f022065f90d3d9ccf92ee8aecdc` requires exact current endpoint membership and two complete readiness rounds covering every expected Pod UID and server worker before warmup; workload and thresholds are unchanged. A wholly fresh governed artifact/Q0/54-calibration suite is next; the 78-run credit matrix and Integrated V4 remain not started. |
