...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Four pre-Q0 suites remain immutable zero-credit. Suite `x1-canonical-20260829t202813z-1ce8ccdc` completed governed artifacts but exposed missing WSL GPU mounts and partial-rollout/file-SD cleanup defects before Q0. Manual cleanup restored B0 CUDA, Prometheus 5/5, queue 0/0/0 and zero X1 runtime/lease/schema residue. Minimal correction/refreeze and a wholly fresh Q0/calibration suite are required; 78-run credit remains not started. |
