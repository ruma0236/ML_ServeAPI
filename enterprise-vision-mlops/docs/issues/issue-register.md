...
| `S7` | `EVM-297` | Image/VLM/LLM Auxiliary Admission | Implementing | Post-W8 | Jira `SCRUM-207`. Existing image and generative serving paths enforce a versioned family-cost admission contract. The strict diagnostic gate passed 18/18 requests and exact cleanup; the first full matrix remains zero-credit RCA after a positional projection defect. A clean second 36-repetition external-HTTP real-CUDA matrix passed independent raw-derived validation with 162 completed requests, 54 intentional bounded rejections, zero OOM/starvation, complete trace/Prometheus/quality/fairness evidence, and exact cleanup. Full regressions and canonical closure remain pending; `S7-AC-01..04` are not yet closed. |
