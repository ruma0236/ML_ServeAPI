...
|---|---|---|---|---|
| `EVM-286` | Generic scenario workload lifecycle contract and RTX 4080 executor | In Progress / contract core implemented | Post-Sprint 178 | Jira `SCRUM-195` under Epic `SCRUM-194`; `scenario_workloads.py` adds exact identity, dependency-aware state, exclusive fenced GPU lease and final evidence sealing. Focused/scenario regressions 11/11, Control Panel contract 8/8 and Ruff pass. API/worker and real adapter invocation remain open; no model run is credited yet. |
