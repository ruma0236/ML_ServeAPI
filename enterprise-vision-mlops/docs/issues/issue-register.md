...
| `EVM-244` | A/B canary traffic router and evaluator | In Progress | 2026-07-W8 | deterministic 500-pair shadow and exact 100/1,000 isolated replay routing, ordered quality/runtime guardrails, zero allocation, exact B0 route restoration and common evidence reports are implemented. Contract audits added mandatory post-replay B0 inference, a versioned serving-root config, zero stable-authoritative HTTP errors and typed stable-success invariants; 53 operational tests pass. Fresh real VisA/CUDA closure runs remain pending; production traffic and business A/B remain out of scope |
...
| `EVM-267` | Scenario B invalid model canary and rollback | In Progress | Sprint 178 | B0-B5 are implemented with real immutable B7 quality blocker, deterministic <=10% isolated replay, exact rollback, canonical F-drive hashes and common live-proof reports. Earlier URI-defect, postcondition-incomplete and stable-serving-path candidates remain immutable RCA evidence. `docs/status/2026-08-02-scenario-b-stable-replay-uri-rca.md` records that all four post-RCA candidates had 1,000/1,000 failed stable HTTP observations; versioned serving root, zero stable errors and exact post-inference are now required. Fresh B6 runs remain necessary. Production rollout stays fail-closed until Scenario D, dual-model GPU admission and separate maintenance approval; no business A/B claim. |
