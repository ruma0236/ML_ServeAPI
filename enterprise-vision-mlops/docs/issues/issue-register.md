...
| `EVM-EPIC-25` | Distributed Scale And Operational Load Validation | Post-W8 | In Progress | S0-S7 are verified after strict post-closure audit; S8 soak/resource-efficiency closure remains planned and not started. Single-node limits remain explicit. |
...
| `S8` | `EVM-298` | Dependency Soak & Resource-efficiency Closure | Implementing | Post-W8 | external runner and independent raw/Git-blob validator are implemented. Attempt 01 retained a pre-admission Prometheus host-port TOCTOU race; attempt 02 passed nine fresh control/latency/transient repetitions but exposed runner/config drift and five expiry outcomes in retry-budget repetition 1. Both attempts have zero acceptance credit and complete cleanup. A separate zero-credit v2 calibration closed 17/17 tasks with duplicate effects zero and measured 40.109 seconds to terminal recovery. The v3 profile therefore reads the declared 12 transient + 4 healthy workload, freezes an 8-per-60-second global budget below circuit throughput, records monotonic fault-to-terminal elapsed time, and sets the pre-acceptance MTTR guardrail to 60 seconds. A fresh 21-repetition matrix, 30-minute x3 soak, regressions and hash closure remain pending. |
