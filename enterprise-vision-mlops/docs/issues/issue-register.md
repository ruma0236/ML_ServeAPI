...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Contract Frozen / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Suite `x1-canonical-20260830t071730z-6f48e7ee` remains immutable zero-credit after Q0 `4/4` and LogisticRegression solo `3/3`; GaussianNB rep1 then failed the strict enqueue-window gate. The historical source did not preserve the violating window, so the exact step/delta is independently unverifiable. Private inventory is `48` files / `489,418,771` bytes, aggregate `d6ca69fd...c7911`, index `da540190...d17a2`; cleanup is exact. Revision `5428ba2dfe01ff24c5370e8cd6a0685c97b93d30` retains every workload/threshold/matrix value, removes per-tick pending-future scans, and preserves any future strict-window violation plus observability before rejection. A wholly fresh governed artifact/Q0/54-calibration suite is next; the 78-run credit matrix and Integrated V4 remain not started. |
