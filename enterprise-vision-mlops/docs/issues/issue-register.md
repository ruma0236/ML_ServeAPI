...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Six pre-Q0 suites remain immutable zero-credit. The sixth reached CUDA-ready Triton but the API validator expected string version `"1"` while frozen config and Triton protobuf JSON use numeric `1`, so readiness failed closed before Q0. Exact protobuf-version remediation/refreeze and a wholly fresh artifact/Q0/calibration suite are required; 78-run credit remains not started. |
