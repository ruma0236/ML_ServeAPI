...
| `S2` | `EVM-293` | Bounded Queue & Backpressure | Implementing | Post-W8 | revision `5de1c41` closes audited pre-FastAPI ingress, approval bounds, canonical depth/bytes, idempotency tombstones, deadline and effect fencing, fair runtime polling, outcome-unknown disposition, cutover/mirror parity, exact parent-bound executor cleanup, portable process-tree RSS, CPU backlog scaling, and global GPU outstanding=1 in the existing API/worker path. `38` focused, `661` full Python, and `59` Control Panel tests plus frontend build and Compose validation passed. A-J external profiles x3, trusted nonzero CUDA activity, Prometheus/OTLP closure, and all S2 ACs remain pending. |
...
EVM-290 and EVM-292 are verified from canonical evidence. EVM-293 is
implementation-only with no accepted AC credit; EVM-291 and EVM-294..298
