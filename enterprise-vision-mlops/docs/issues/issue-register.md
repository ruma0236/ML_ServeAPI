...
| `S3` | `EVM-291` | HIGGS Lightweight Capacity Envelope | Implementing | Post-W8 | Jira `SCRUM-201`. The existing Workloads API exposes a versioned tabular capacity catalog and prediction route for the frozen five families. Dataset, split, model artifact, and request identities fail closed; validation/transform/prediction spans and bounded metrics are wired. A versioned streaming preparation path now preserves the official 10.5M/0.5M boundary, deterministic bounded samples, and five numeric artifact contracts. The official source is held outside Git at `2,816,407,858` bytes with observed SHA-256 `ea302c18...3b698`. Synthetic preparation/runtime and focused API/compatibility/metrics/ledger tests plus Ruff and the canonical validator pass. Full preparation and external load have not run; all S3 ACs remain pending. |
