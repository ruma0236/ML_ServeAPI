...
| `EVM-300` | S6B-M Triton Model Blue/Green | Remediation Required / Jira `SCRUM-211` In Progress | EVM-299 verified | Two pre-acceptance attempts remain immutable with 0 acceptance requests: missing log path `b94ec797...f477b`, then uppercase run identity/file-SD cleanup `b5edb5a1...31c10a`. B0 CUDA, lease/container/targets, and Prometheus 5/5 were restored after each. Lowercase run IDs and empty-set-before-delete Prometheus cleanup are being committed before restarting the full matrix; all AC remain pending. |
