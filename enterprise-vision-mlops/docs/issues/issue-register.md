...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Contract Frozen / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Historical failures and diagnostics remain immutable zero/non-credit. Suite `x1-canonical-20260830t005658z-2f302414` passed Q0 `4/4` (`256` actual-CUDA requests), then received zero credit when its first solo projection found no safe step. The old projection-before-write order omitted the exact six-step raw bundle, so that result is independently undiagnosable. Revision `5b4ac7da88dae0c26d1beb385cf34b43a1f9dbae` now preserves complete raw calibration evidence before projection without changing workload or thresholds. A wholly fresh governed artifact/Q0/54-calibration suite is next; the 78-run credit matrix and Integrated V4 remain not started. |
