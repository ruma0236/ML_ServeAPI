...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Twelve failed suites remain immutable zero-credit. Suite `x1-canonical-20260829t230109z-3e7df802` timed out on DLRM request 0001 through both NodePort and direct Kubernetes port-forward; Triton retained one pending request with execution still one. Bounded direct-image/trace-on/trace-off isolation is frozen at `7ffda8b0...991503` with focused `62/62`; Q0/54 calibrations and 78-run credit remain not started. |
