...
| `EVM-266` | Scenario A GPU and serving failure recovery | Complete; Jira closure pending sync | Sprint 178 | A0-A8 complete: 49 focused tests and Ruff pass; real production-candidate CUDA CT has 2,181 holdout records and zero overlap; three independent UID-preconditioned B0 Pod recovery runs detected in 0.170-0.200 s, interrupted the endpoint for 9.872-9.904 s, recovered in 10.082-25.087 s, restored exact identity and CUDA inference, and returned the exact Prometheus target to up; the earlier malformed-path run remains immutable RCA evidence |
