...
| `S5` | `EVM-296` | Criteo Spark Memory-bounded Data Scale | Implementing (strict reclosure) | Post-W8 | Jira `SCRUM-206`. Historical closure `5aff042` and the immutable accepted 30-point matrix are preserved, but an independent audit found that closure v1 trusted regression, runtime-smoke, and cleanup summary booleans. Current remediation independently binds runtime-smoke Git bytes and measurements, command/exit/test-count/log hashes, cleanup counts, and mutation tests. The accepted matrix is not being rerun unless its source/runtime identity or private 57-artifact inventory fails. Peak Spark executor memory is `150,994,944` bytes; the separate single-process columnar peak is `489,484,288` bytes. The skew guardrail change from `200` to `400` was pilot-informed pre-acceptance tuning after an observed `280.18` ratio and is not a sensitivity proof. S5 remains unverified until the new strict closure passes. |
| `S6` | `EVM-295` | API Rolling Continuity & GPU Controlled Handoff | Verified (evidence hardening) | Post-W8 | Jira `SCRUM-205`. Three isolated two-replica API rolling repetitions completed `1,500/1,500` logical requests with accepted loss `0`, duplicate effects `0`, exact old/new Pod and image identity, drain events, complete sampled W3C traces, and measured rollout recovery of `16.313-20.969 s`; p99 was `172.0-1,265.01 ms`. The final repetitions prove exact UID drain events under traffic, but their maximum drain waits were only `2-3 us`; the separate preflight proves one actual approximately 2-second in-flight drain. This is measured rolling continuity, not a claim that all three final repetitions waited on long in-flight requests. Mutation tests now fix fail-closed handling for trace gaps, drain UID mismatch, GPU owner overlap, rollback mismatch, and approval reuse. The controlled single-GPU handoff result and local single-node claim boundary are unchanged. |
| `S7` | `EVM-297` | Image/VLM/LLM Auxiliary Admission | Implementing (strict reclosure) | Post-W8 | Jira `SCRUM-207`. Historical closure `3ec3039` and its immutable 36-run matrix are preserved. An independent audit found that closure v1 did not recompute its numerical totals and used ambiguous starvation wording: the raw matrix has 162 completed requests, 54 intentional over-limit pre-admission rejections, selected/admitted starvation `0`, and full-matrix long noncompletion `54`. Remediation separates those scopes, binds family source/revision/digest/license/provenance/cache manifests, records exact all-family `/ready` identity, and requires observed LLM 4-bit loading. ScienceQA-derived VLM evidence is non-commercial portfolio/research evidence only under CC-BY-NC-SA-4.0. S7 remains unverified until reprojected evidence, current-revision CUDA smoke, mutation tests, regressions, and cleanup pass. |
...
EVM-290, EVM-291, EVM-292, EVM-293, EVM-294, and EVM-295 are verified from canonical evidence. EVM-296 and EVM-297 are reopened for strict evidence reclosure; their historical accepted matrices and closures remain immutable and receive no new acceptance credit until the new validators pass.
...
its failed attempts and rollback boundary remain independent operational evidence. EVM-295 remains verified with the narrowed drain-evidence claim above. EVM-296 and EVM-297 are implementing strict reclosure, while EVM-298 remains planning-only.
