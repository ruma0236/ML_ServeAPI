...
| `EVM-EPIC-25` | Distributed Scale And Operational Load Validation | Post-W8 | In Progress | S0-S7 are verified after strict post-closure audit; S8 dependency-fault and soak/resource-efficiency work is implementing with three rejected zero-credit attempts retained. Single-node limits remain explicit. |
...
| `S8` | `EVM-298` | Dependency Soak & Resource-efficiency Closure | Implementing | Post-W8 | external runner and independent raw/Git-blob validator are implemented. Attempts 01-03 retain, respectively, a Prometheus host-port TOCTOU race, retry-budget runner/config drift, and an equal 10-second worker/HTTP timeout ambiguity. A zero-credit v2 calibration closed 17/17 tasks and the v5 timeout preflight later passed 6/6 terminal/trace closure with 51.484-second recovery. Attempt 04 then passed all 21 individual fault scopes but aggregate admission rejected a worker-loss projection that included a sequential timeout/HOL phase and measured 61.608-63.234 seconds against the unchanged 60-second bound. V6 separates exact PID-failure-to-reclaimed-task worker MTTR from full profile elapsed; its zero-credit worker-loss preflight measured 11.954 seconds versus 62.234 seconds full profile elapsed, with terminal/trace 3/3, duplicate effects zero, and cleanup complete. All failed attempts have zero acceptance credit. A fresh 21-repetition matrix, 30-minute x3 soak, regressions and hash closure remain pending. |
...
its failed attempts and rollback boundary remain independent operational evidence. EVM-295 remains verified with the narrowed drain-evidence claim above. EVM-296 and EVM-297 are verified after strict reclosure. EVM-298 is implementing: four rejected full attempts, one zero-credit retry calibration, one rejected v4 timeout preflight, and one passing zero-credit v5 timeout preflight are retained. No acceptance repetition or soak result is credited before a fresh v6 run.
