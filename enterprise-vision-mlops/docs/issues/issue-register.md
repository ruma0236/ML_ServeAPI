...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Contract Frozen / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Append-only model/topology amendment freezes four fresh governed models, actual API replicas 1/2 and server CPU workers 1/2/4, 12 solo + 18 topology + 24 batching non-credit calibrations, and the unchanged future 78-run matrix. Preliminary X1 evidence is isolated and forbidden from reuse; physical Q0/calibration and all acceptance ACs remain pending. |
