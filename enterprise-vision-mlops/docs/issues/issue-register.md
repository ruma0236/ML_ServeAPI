...
| `EVM-277` | Common lifecycle identity envelope, guard dispatcher and golden path | In Progress | Post-Sprint 178 | Jira `SCRUM-185`; common guard and single-use GPU handoff approval implemented; 436 tests at `4d716a2`; stale profile and cross-runtime path failures remained fail-closed; attempt `lifecycle-20260802T163003-ec64fa8c` passed 11/11 preflight plus Airflow 18/18, then exposed a historical-Pod false blocker during handoff; automatic B0 recovery passed, exact active-Pod identity remediation is implemented, and a new revision-bound L0-L7 attempt remains required |
