...
| `EVM-226` | Docker Desktop Kubernetes B7 training and serving proof | In Progress | 2026-07-W7 | Docker Desktop Kubernetes is running and the B7 Job/Deployment, F-drive PV/PVCs, probes, immutable image/model digests, real VisA Docker GPU inference, and fail-closed proof runners are implemented; both PVCs bind, but the embedded kubeadm/cri-dockerd node does not advertise `nvidia.com/gpu`, so the digest-pinned Job remains Pending with `Insufficient nvidia.com/gpu` and Kubernetes training, serving, controlled failure, and rollback are not complete; evidence: `docs/status/2026-07-10-w7-kubernetes-b7-execution-proof.md` |
