...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Contract Frozen / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Historical failures and diagnostics remain immutable zero/non-credit. Suite `x1-canonical-20260830t001821z-12bffcd6` passed saved-artifact CUDA repeatability and Q0 `4/4` (`256` requests) but received zero credit when underscore-bearing runtime IDs caused HTTP `422 x250` in the first warmup. Revision `559d267509e1e23f2a478fc968dc3886133ee036` refreezes hyphen-only API-compatible IDs and failed-warmup evidence persistence without changing workload or thresholds. A wholly fresh governed artifact/Q0/54-calibration suite is next; the 78-run credit matrix and Integrated V4 remain not started. |
