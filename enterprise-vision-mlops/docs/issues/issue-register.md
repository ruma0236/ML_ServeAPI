...
| `S8` | `EVM-298` | Dependency Soak & Resource-efficiency Closure | Implementing | Post-W8 | external runner and independent raw/Git-blob validator are implemented. Attempts 01-03 retain, respectively, a Prometheus host-port TOCTOU race, retry-budget runner/config drift, and an equal 10-second worker/HTTP timeout ambiguity; all have zero acceptance credit and complete cleanup. A zero-credit v2 calibration closed 17/17 tasks with duplicate effects zero and measured 40.109 seconds to terminal recovery. The later v4 timeout preflight reached 6/6 terminal outcomes with outcome-unknown and duplicate effects both zero, but failed strict trace completeness at 2/6 and the 60-second MTTR guardrail at 72.828 seconds because the injected delay also affected four healthy requests. V5 isolates delay to timeout work while retaining exact failed-terminal, zero-effect, bounded-observation semantics. A passing zero-credit v5 timeout preflight, fresh 21-repetition matrix, 30-minute x3 soak, regressions and hash closure remain pending. |
...
its failed attempts and rollback boundary remain independent operational evidence. EVM-295 remains verified with the narrowed drain-evidence claim above. EVM-296 and EVM-297 are verified after strict reclosure. EVM-298 is implementing: three rejected full attempts, one zero-credit retry calibration, and one rejected zero-credit timeout preflight are retained. No acceptance repetition or soak result is credited before a passing v5 timeout preflight and a fresh v5 run.
