...
| `S5` | `EVM-296` | Criteo Spark Memory-bounded Data Scale | Implementing | Post-W8 | Jira `SCRUM-206`. Governed click-log intake now fixes 4 public shards into small/medium/large stages on F-drive storage. One 766,864-row non-acceptance preparation smoke produced the same digest with missing/duplicate zero through single-process columnar, local Spark, and one Kubernetes executor; scoped RBAC, event-log telemetry, and exact cleanup passed. Full 3-repeat staged, executor 1/2/4, executor-loss, idempotent replay, regression, and hash closure remain pending. |
...
EVM-290, EVM-291, EVM-292, EVM-293, and EVM-294 are verified from canonical evidence.
...
implementing with preparation-only evidence; EVM-295, EVM-297, and EVM-298 remain planning-only.
