...
| `S5` | `EVM-296` | Criteo Spark Memory-bounded Data Scale | Verified | Post-W8 | Jira `SCRUM-206`. The accepted clean-revision matrix completed `30` points across single-process columnar, local Spark, Kubernetes executors `1/2/4`, and three exact executor-loss replays. Normal Kubernetes means were `143,826.84`, `200,076.779`, and `220,778.666` records/s for executor counts `1/2/4`; peak executor memory was `150,994,944` bytes, maximum GC ratio `0.06634`, and maximum measured skew ratio `280.18`. Missing/duplicate records were `0/0`; retry preserved the `3,061,802` semantic-row digest while `12,247,208` generated-I/O rows remained explicitly non-semantic. `16` focused, `789` full Python with real PostgreSQL, `162` lifecycle/host, and `77` Control Panel tests plus the production frontend build passed. Strict Git-byte/public-private evidence validation, current-revision smoke, exact cleanup, Prometheus `4/4`, and serving `1/1` passed. Two rejected experiment attempts and one regression-command RCA remain preserved without acceptance credit. Local single-node controlled evidence only; not customer traffic, production SLA, physical multi-node, full-terabyte, HA/DR, or multi-zone proof. |
| `S6` | `EVM-295` | API Rolling Continuity & GPU Controlled Handoff | Verified | Post-W8 | Jira `SCRUM-205`. Three isolated two-replica API rolling repetitions completed `1,500/1,500` logical requests with accepted loss `0`, duplicate effects `0`, exact old/new Pod and image identity, exact drain evidence, complete sampled W3C traces, and measured rollout recovery of `16.313-20.969 s`; p99 was `172.0-1,265.01 ms`. One calibration and three independent single-GPU source-to-target-to-source handoffs preserved owner overlap `0`, exact single-use approval/candidate/rollback identity, real CUDA inference, and measured interruptions of `9.125-10.281 s` and `10.031-11.234 s`. `34` focused, `72` real-PostgreSQL control-plane, `807` full Python, `159` lifecycle/host, and `59` Control Panel tests plus the production frontend build passed. Two rejected runtime attempts and regression-command RCA remain without acceptance credit. All isolated S6 resources and images were removed; source serving returned `1/1`, target `0`, queues/leases `0`, and Prometheus `5/5`. Local single-node controlled evidence only; this is not customer traffic, SLA, physical-node/multi-zone HA/DR, zero-downtime GPU HA, or multi-GPU proof. |
| `S7` | `EVM-297` | Image/VLM/LLM Auxiliary Admission | Implementing | Post-W8 | Jira `SCRUM-207`. Existing image and generative serving paths enforce a versioned family-cost admission contract. An external-HTTP real-CUDA runner uses the existing exact GPU lease, B0 handoff/restore boundary, Prometheus file discovery, process-tree resource sampling, and raw-derived evidence validator. The strict diagnostic gate passed 18/18 requests and exact cleanup. The first full 36-repetition matrix remains uncredited: runtime results and cleanup completed, but strict validation rejected position-sensitive public/private profile ordering. Stable profile/repetition identity comparison and duplicate fail-closed tests are implemented; a clean full rerun is required and `S7-AC-01..04` remain pending. |
...
EVM-290, EVM-291, EVM-292, EVM-293, EVM-294, EVM-295, and EVM-296 are verified from canonical evidence.
...
its failed attempts and rollback boundary remain independent operational evidence. EVM-296 is
verified by the strict S5 experiment and closure artifacts; EVM-295 is verified by the strict S6 experiment and closure artifacts. EVM-297 is implementing with no acceptance credit yet, while EVM-298 remains planning-only.
