...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Contract Frozen / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Historical failures and diagnostics remain immutable zero/non-credit. Suite `x1-canonical-20260830t013829z-fdefc2c7` passed Q0 `4/4` (`256` actual-CUDA requests) and two solo repetitions, then received zero credit when rep3 produced one committed durable effect paired with HTTP `503` and no terminal trace. Private inventory remains `47` files / `738,932,667` bytes, aggregate `8fd5f530...cd3d`, index `63f7b5d5...520cd`. Revision `c273e2ad140a945b35370540c6894f9eac1b7cb1` separates generic X1 effect readback from the S6B-M-only clock receipt while retaining synchronous commit and exact separate-transaction parity. A wholly fresh governed artifact/Q0/54-calibration suite is next; the 78-run credit matrix and Integrated V4 remain not started. |
