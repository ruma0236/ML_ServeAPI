...
| `EVM-235` | CI-gated deployment intent and state machine | In Progress | 2026-07-W7 | Root GitHub CI, immutable CI evidence validation, server-side admission, optimistic-lock audit ledger, production approver/namespace enforcement, isolated executor, failure/rollback branches, live Control Panel controls, 2-second aggregation cache, 82 Python tests, 19 frontend contracts, 14 desktop/mobile E2E scenarios, and 24-request load proof are implemented. Live API correctly returns `409 ci_evidence_missing`; real `queued -> applying -> applied/rolled_back` closure remains blocked by EVM-226/EVM-236 readiness and a downloaded successful GitHub CI bundle. |
