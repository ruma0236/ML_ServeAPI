...
| `EVM-226` | Docker Desktop Kubernetes B7 training and serving proof | In Progress | 2026-07-W7 | Docker Desktop WSL2 GPU bridge now advertises `nvidia.com/gpu=1` through pinned NVIDIA device plugin `v0.18.0`; a non-privileged Pod requested the extended resource and reported the RTX 4080 SUPER. The bridge, dynamic WSL driver discovery, read-only workload patches, and F-drive evidence are verified in `docs/status/2026-07-10-w7-docker-desktop-kubernetes-gpu-bridge.md`. EVM-226 remains open until the selected B7 Job, serving rollout, inference, controlled failure, and rollback complete. |
