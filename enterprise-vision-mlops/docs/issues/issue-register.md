...
| `EVM-226` | Docker Desktop Kubernetes B7 training and serving proof | In Progress | 2026-07-W7 | Docker Desktop Kubernetes is running and the B7 Job/Deployment, F-drive PV/PVCs, probes, immutable image/model digests, real VisA Docker GPU inference, and fail-closed proof runners are implemented. Both PVCs bind, but the node does not advertise `nvidia.com/gpu`; the Job was initially unschedulable with `Insufficient nvidia.com/gpu` and is now `Failed / DeadlineExceeded` after its 7,200-second active deadline. Kubernetes training, serving, controlled failure, and rollback are not complete. Live status is visible through the EVM-229 observer follow-up in `docs/status/2026-07-10-w7-live-kubernetes-resource-observer.md`. |
...
| `EVM-228` | Compressed W6/W7 integration review | In Progress | 2026-07-W7 | real data/model, UI evidence, EVM-236 evaluator, EVM-233 policy, and the EVM-235 CI/deployment-intent implementation are complete; final closeout still requires EVM-226 GPU runtime proof, a live EVM-235 apply/rollback after readiness passes, and the EVM-234 measured review-first drift event |
| `EVM-229` | Kubernetes resource topology and animation UI | Done | 2026-07-W7 | UI visualizes namespace/node/pod/job/service/PVC/GPU state with readable transitions and drilldowns for allocation, pressure, restarts, readiness, and dry-run action labels. Commit `a4d318a` replaces projection-only interpretation with a sanitized five-second Kubernetes observer, explicit live/stale/projected source metadata, state-change history, Live/All UI modes, and real `evm-b7-training = Failed / DeadlineExceeded` evidence; verified by `docs/status/2026-07-10-w7-live-kubernetes-resource-observer.md`. |
...
| `EVM-234` | B7 drift review event | In Progress | 2026-07-W7 | UI surface is complete, but queue-count heuristic is not accepted as drift proof; compare selected B7 validation baseline with a distinct real-input prediction/confidence window and emit `review_required` for label review/approval without automatic retraining |
| `EVM-235` | CI-gated deployment intent and state machine | In Progress | 2026-07-W7 | EVM-235 implementation GitHub CI run `29087199282` and deployment-admission run `29087230617` passed; both artifact archives were SHA-256 verified and imported to the F-drive. Immutable CI validation, server admission, optimistic-lock audit ledger, production approver/namespace enforcement, isolated executor, fail-before-side-effect rollback guard, live Control Panel controls, a 2-second aggregation cache, 85 Python tests, 19 frontend contracts, 14 desktop/mobile E2E scenarios, and 24-request load proof are implemented. Live CI/CD are `pass`; creation correctly returns `409` for `artifact_readiness_not_ready`, `environment_policy_blocked`, and `rollback_reference_not_ready`. Real `queued -> applying -> applied/rolled_back` closure remains blocked by EVM-226/EVM-236, so the issue remains In Progress. Jira comments `10233` and `10234`, Notion page `39910ad2-dcad-8178-8a2a-eee87e55d719`, and the linked Obsidian EVM-235 work log preserve the same boundary. |
| `EVM-236` | Enterprise evidence readiness evaluator | Done | 2026-07-W7 | Artifact-content evaluator, deterministic validator, CycleRun/OpenAPI/UI integration, and producer guards are implemented. Live evidence report `readiness-d7849f6b195bd356` is structurally valid and correctly returns `blocked` from 13 checks and 14 evidence-level blockers; this closes the evaluator implementation, not model promotion readiness. |
| `EVM-237` | Torch EfficientNet-B0/B7 real model matrix | Done | 2026-07-W7 | all four Torch/TorchVision EfficientNet candidates ran on real VisA split `10821/6504/2136/2181`, produced MLflow runs, F-drive artifacts, metrics, training histories, confusion matrices, GPU profiles, environment reports, model cards, and populated `CycleRun.model_matrix`; best candidate is `effnet-b7-img600-finetune-adamw` with MLflow run `a4e2763b28ae494ea67944084edd4b3f`, accuracy `0.961027`, F1 `0.812362`, AUROC `0.970650`; verified in `docs/status/2026-07-10-w7-real-test-evidence-validation.md` |
