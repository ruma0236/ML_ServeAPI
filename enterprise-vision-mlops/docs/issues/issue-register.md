...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Contract Frozen / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Historical failures and diagnostics remain immutable zero/non-credit. Suite `x1-canonical-20260830t003358z-3776544d` passed saved-artifact CUDA repeatability and Q0 `4/4` (`256` requests) but received zero credit when all `250` accepted first-warmup requests failed HTTP `503` before durable effect commit. Real PostgreSQL reproduced the X1 event entering the S6B-M-only causal validator. Revision `54d92caa8b309bd58560d3d59fa0a23c8212990c` refreezes the generic X1 durable receipt path and distinct failure attribution without changing workload or thresholds. A wholly fresh governed artifact/Q0/54-calibration suite is next; the 78-run credit matrix and Integrated V4 remain not started. |
