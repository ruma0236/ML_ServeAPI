...
| `EVM-300` | S6B-M Triton Model Blue/Green | Running / Jira `SCRUM-211` In Progress | EVM-299 verified | Five failed suites remain immutable and zero-credit. Remediation `c1618a4` resolves and validates config bytes from the parent Git root before mutation and passed 21 focused tests. The complete matrix restarted from that clean revision; all AC remain pending/non-credit. |
