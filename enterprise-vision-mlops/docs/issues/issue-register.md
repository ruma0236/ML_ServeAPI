...
| `EVM-279` | Scenario D idempotent lifecycle continuity and side-effect reconciliation | In Progress | Post-Sprint 178 | Jira `SCRUM-187`; exact Job reconciliation and ledger guards pass non-disruptive replay `scenario-d-lifecycle-20260802T182104Z-fdcf0047` with 18 decisions, zero external identity delta and 62/62 hashes. Exact worker/observer/worker live series has detection max 5.212014 s, recovery max 8.6442343 s and 24/24 hashes. Combined attempts exposed and retained three harness RCAs before worker termination: missing phase approval, stale run-label formula after real Job admission, and a transient pre-persistence Job `NotFound`. Automatic cancellation restored B0 CUDA in 48 s and 10 s for the mutating attempts. The runner now binds exact approvals, uses canonical manifest identity, and waits only on `NotFound`; 465/465 tests pass. A fresh combined attempt remains required, so D stays open and no distributed exactly-once claim is made. |
