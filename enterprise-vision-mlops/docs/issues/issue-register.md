...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Contract Frozen / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Historical failures and diagnostics remain immutable zero/non-credit. Suite `x1-canonical-20260830t025102z-a5a34711` passed Q0 `4/4` (`256` actual-CUDA requests) and four solo repetitions, then received zero credit when GaussianNB rep2's `400 RPS` warmup retained one localhost `WinError 10048` outcome-unknown among `4,000` offers; `1,574` completed requests match `1,574` durable effects and `2,425` offers were bounded local rejections. Private inventory is `51` files / `643,037,142` bytes, aggregate `861cf7a5...109a`, index `1bde6695...8ae2`. Revision `4034732cbb36c934eade8e5b1eb5b71fd3ef6bae` uses one retry-free pool-size-one keep-alive session per client driver thread per traffic window; retry, timeout, workload, thresholds, Q0, and matrices remain frozen. A wholly fresh governed artifact/Q0/54-calibration suite is next; the 78-run credit matrix and Integrated V4 remain not started. |
