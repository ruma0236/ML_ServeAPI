...
| `S7` | `EVM-297` | Image/VLM/LLM Auxiliary Admission | Implementing | Post-W8 | Jira `SCRUM-207`. Existing image and generative serving paths enforce a versioned family-cost admission contract. An external-HTTP real-CUDA runner uses the existing exact GPU lease, B0 handoff/restore boundary, Prometheus file discovery, process-tree resource sampling, and raw-derived evidence validator. The first image warmup was rejected after host input remapping; three later family diagnostics completed but were also rejected with zero credit because cleanup captured a transient Prometheus 0/0 restart state. The runner now freezes and requires the exact 5/5 baseline before cleanup can pass. Full sequential CUDA profiles remain uncredited; `S7-AC-01..04` are pending. |
