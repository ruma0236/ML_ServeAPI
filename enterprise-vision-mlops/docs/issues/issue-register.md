...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Two fresh suites are immutable pre-Q0 zero-credit history. Import-order remediation passed, then `x1-canonical-20260829t200526z-384959cd` fail-closed on loader names that did not match the governed Criteo `int_feature`/`cat_feature` schema. Cleanup passed. Minimal schema-name alignment, actual-shard/negative tests, refreeze, and a fresh full calibration are required; 78-run credit remains not started. |
