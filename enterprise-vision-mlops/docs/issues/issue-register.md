...
| `EVM-244` | A/B canary traffic router and evaluator | In Progress | 2026-07-W8 | deterministic 500-pair shadow and exact 100/1,000 isolated replay routing, ordered quality/runtime guardrails, zero allocation, exact B0 route restoration, common evidence reports and 52 operational tests are implemented; final source-bound real VisA/CUDA runs remain pending after the postcondition audit added mandatory post-replay B0 inference; production traffic and business A/B remain out of scope |
...
| `EVM-267` | Scenario B invalid model canary and rollback | In Progress | Sprint 178 | B0-B5 are implemented with real immutable B7 quality blocker, deterministic <=10% isolated replay, exact rollback, canonical F-drive hashes and common live-proof reports. The first URI-defect runs and the later postcondition-incomplete candidates remain immutable RCA evidence. `docs/status/2026-08-02-scenario-b-postcondition-contract-audit.md` adds mandatory post-replay B0 inference; fresh B6 runs remain required. Production rollout stays fail-closed until Scenario D, dual-model GPU admission and separate maintenance approval; no business A/B claim. |
