...
| `S8` | `EVM-298` | Dependency Soak & Resource-efficiency Closure | Implementing | Post-W8 | external runner and independent raw/Git-blob validator are implemented. Attempt 01 retained zero acceptance credit: two controls passed, control repetition 3 hit an isolated Prometheus host-port TOCTOU bind collision before admission, cleanup passed, and soak did not start. A bounded collision-only retry remediation is pending clean rerun; 21 fresh fault repetitions, 30-minute x3 soak, regressions and hash closure remain pending. |
