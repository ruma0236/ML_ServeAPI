...
| `EVM-282` | Scenario A post-promotion GPU serving recovery | In Progress / first live attempt failed / M0 recovered / remediation pending fresh proof | Post-Sprint 178 | Jira `SCRUM-190`, comment `10557`; attempt `scenario-a-lifecycle-20260803T000920Z-0209bac1-47eaa66e` verified M1 then failed on a 275-character Windows atomic state path. Exact delayed rollback `rollback-recovery-20260803T001831Z` restored M0/CUDA/two Prometheus scrapes/pointer without acceptance credit. Git `9c98bfd` adds compact path preflight and exact Recreate reconciliation for a fresh immutable attempt. |
...
Current lifecycle execution evidence has closed E, D, C and B. Scenario A is
still in progress after an immutable failed attempt and successful delayed M0
recovery; cross-scenario correlation remains blocked on a fresh A PASS. C's
