...
| `S7` | `EVM-297` | Image/VLM/LLM Auxiliary Admission | Implementing | Post-W8 | Jira `SCRUM-207`. Existing image and generative serving paths enforce a versioned family-cost admission contract. An external-HTTP real-CUDA runner now uses the existing exact GPU lease, B0 handoff/restore boundary, Prometheus file discovery, process-tree resource sampling, and raw-derived evidence validator. Focused contract tests pass, but diagnostic and full sequential CUDA profiles remain uncredited; `S7-AC-01..04` are pending. |
