...
| `S4` | `EVM-294` | HIGGS Tiny MLP GPU Batching | Implementing | Post-W8 | batch-one preparation passed; per-request traces closed in three pilot runs, but concurrency 128 exposed a client p99 hard-stop breach and was reduced to frozen 64; the 64-concurrency pilot, fresh 60+3+3 matrix and all ACs remain pending |
...
EVM-294 is implementing from a hash-linked batch-one preparation checkpoint; its
full 60+3+3 matrix and all acceptance criteria remain pending. EVM-295..298
