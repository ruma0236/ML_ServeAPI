...
| `EVM-268` | Scenario C quality degradation and retraining gate | In Progress | Sprint 178 | C0 contract pins the current VisA/B0/shard/CT identities, deterministic category-mix shift, signal thresholds, idempotent event/candidate rules, hold/reject/approval audit, and zero-auto-promotion boundary. Non-disruptive C implementation proceeds by explicit priority change, while Scenario E remains a hard blocker for any limited release or deployment intent |
