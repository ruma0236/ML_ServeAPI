...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | All failed suites/diagnostics remain immutable zero/non-credit. Fresh diagnostic `x1-diagnostic-20260829t235715z-e3be2097` reproduced one correct call then a stall in host CUDA, four-model Triton trace-on/off, and DLRM-only Triton; tracing and co-residency are not differentiators. A fresh governed DLRM-lite compatibility artifact is next; Q0/54 calibrations and 78-run credit remain not started. |
