...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Seven pre-Q0 suites remain immutable zero-credit. Suite `x1-canonical-20260829t212625z-c63435bf` reached CUDA-ready Triton but failed API-to-Triton startup connection readiness before Q0; cleanup restored B0 CUDA, Prometheus `5/5`, queues `0/0/0`, and zero runtime/lease/schema residue. Only readiness-client startup recovery may be corrected before a wholly fresh artifact/Q0/54-calibration suite; 78-run credit remains not started. |
