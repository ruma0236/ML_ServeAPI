...
| `EVM-281` | Scenario B lifecycle candidate release and stable rollback | In Progress | Post-Sprint 178 | Jira `SCRUM-189`; exact-run release guard, immutable profile requirement, evidence hash closure, lifecycle/candidate/model/CT/source binding and two-branch real runner implemented. Initial invocation stopped before LifecycleRun creation because Python discovery selected a Torch-free Conda base; external effects were zero. Wrapper fallback now targets the established F-drive CUDA runtime. Fresh real quality rejection plus 1,000-request controlled runtime breach remain required; no closure yet. |
