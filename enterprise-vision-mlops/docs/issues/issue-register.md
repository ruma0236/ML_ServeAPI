...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Eleven failed suites remain immutable zero-credit. Shared-session suite `x1-canonical-20260829t223852z-08a316f1` reproduced the DLRM request-0001 NodePort timeout; Triton counters prove no pending/execution/failure admission for that request, superseding the per-request-connection hypothesis. Bounded transport diagnosis is required before a wholly fresh artifact/Q0/54-calibration suite; 78-run credit remains not started. |
