...
| `EVM-EPIC-25` | Distributed Scale And Operational Load Validation | Post-W8 | Done | S0-S8 are verified from canonical evidence. S8 closed 21/21 fault repetitions and three independent 30-minute 35 RPS soaks after five rejected zero-credit attempts; single-node and controlled-traffic limits remain explicit. S8-V4 is a separate follow-up umbrella. |
...
| `S4` | `EVM-294` | HIGGS Tiny MLP GPU Batching | Verified | Post-W8 | Jira `SCRUM-204`. A fresh clean-revision `60+3+3` run completed all `66` repetitions through the existing Workloads API and one consumer GPU. Batch `8`, delay `10 ms`, and one model instance measured `160.70` mean saturation RPS with zero OOM; independent no-catch-up confirmation delivered `58.90-59.10 RPS` at `63.87-73.79 ms` p99 and `17.98-20.36 ms` queue-wait p99 with `12/12` sampled trace chains. All four ACs, exact lease/serving restoration, terminal drain, and `69` private artifact hashes passed. The evidence-derived S2 depth recommendation is `4` with rollback `64`, and was not silently applied. `33` focused, `768` Python with real PostgreSQL, `144` lifecycle/host, and `59` Control Panel tests plus the production frontend build passed. The separate 38-task historical cutover mismatch was subsequently reconciled through exact snapshot, dry-run, transactional cancellation and rollback-mirror verification without changing the S4 verdict. Local single-node/single-GPU controlled proof only. |
| `S5` | `EVM-296` | Criteo Spark Memory-bounded Data Scale | Verified (strict reclosure) | Post-W8 | Jira `SCRUM-206`. Historical closure `5aff042` is preserved as superseded v1 evidence. Strict v2 closure `d585ed8` (`e8d5e85f...7a849`) independently revalidates the immutable accepted 30-point matrix, 57 private artifacts, exact 766,864-row three-engine smoke, regression log hashes, and cleanup without rerunning the matrix. Peak Spark executor memory is `150,994,944` bytes; the separate single-process columnar peak is `489,484,288` bytes. The skew guardrail change from `200` to `400` remains pilot-informed pre-acceptance tuning after an observed `280.18` ratio and is not a sensitivity proof. |
| `S6` | `EVM-295` | API Rolling Continuity & GPU Controlled Handoff | Verified (strict evidence hardening) | Post-W8 | Jira `SCRUM-205`. Strict v2 closure `d585ed8` (`fb3dc5c8...8c31`) independently recomputes raw request trace headers and monotonic GPU interruption timelines. Three isolated two-replica API rolling repetitions completed `1,500/1,500` logical requests with accepted loss `0`, duplicate effects `0`, exact old/new Pod and image identity, and rollout recovery of `16.313-20.969 s`; p99 was `172.0-1,265.01 ms`. Final exact-UID drain events measured only `2-3 us`; a separate non-acceptance preflight proves one approximately 2-second in-flight completion. This is controlled local continuity, not HA or proof that all final repetitions waited on long in-flight requests. |
| `S7` | `EVM-297` | Image/VLM/LLM Auxiliary Admission | Verified (strict reclosure) | Post-W8 | Jira `SCRUM-207`. Historical closure `3ec3039` is preserved as superseded v1 evidence. Strict v2 closure `d585ed8` (`30fec691...587bf`) independently recomputes the immutable 36-run matrix: 162 completed requests, 54 intentional over-limit pre-admission rejections, selected/admitted starvation `0`, full-matrix long noncompletion `54`, and OOM/expiry/transport failure `0`. Family source/revision/digest/license/provenance/cache manifests, exact all-family `/ready` identity, observed LLM 4-bit loading, mutation tests, regressions, and cleanup pass. ScienceQA-derived VLM evidence is non-commercial portfolio/research evidence only under CC-BY-NC-SA-4.0. |
| `S8` | `EVM-298` | Dependency Soak & Resource-efficiency Closure | Verified | Post-W8 | Jira `SCRUM-208`. The accepted run closed 21/21 isolated dependency-fault repetitions and three independent 30-minute 35 RPS soaks. Each soak completed 63,000 requests with error rate 0; p99 was 18.85-19.11 ms. S8-AC-01..04, current-revision CUDA/Prometheus/queue cleanup, eight regression gates (1,342 tests passed, 1 skipped), 281 immutable experiment artifacts, and 17 closure artifacts passed canonical Git/private rehash. Five full attempts and all calibration/preflight failures remain zero-credit with RCA. Local single-node/single-GPU controlled evidence only; not customer production SLA, HA/DR, autoscaling, multi-GPU, or simultaneous multi-model residency. |

The canonical planning contract is
`docs/agenda/2026-08-15-distributed-scale-operational-validation-plan-v3.md`.
EVM-290 through EVM-298 are verified from canonical evidence. EVM-296 and EVM-297 strict reclosures preserve their historical accepted matrices and superseded v1 closures; no new matrix acceptance credit was created by evidence reprojection.
EVM-294 retains nine failed-attempt/RCA artifacts and closes only the measured local
single-GPU batching contract. The separate shared-runtime task-queue cutover mismatch
was resolved before S5 through `docs/status/evidence/task-queue-historical-reconciliation.json`;
its failed attempts and rollback boundary remain independent operational evidence. EVM-295 remains verified with the narrowed drain-evidence claim above. EVM-296 and EVM-297 are verified after strict reclosure. EVM-298 is verified from a fresh post-remediation 21-fault and three-soak run; five rejected attempts and all zero-credit calibrations/preflights remain retained and receive no acceptance credit.
...
|---|---|---|---|---|
| `EVM-EPIC-26` | S8-V4 Triton And Heterogeneous-model Operational Validation | Contract Frozen / Jira `SCRUM-209` In Progress | V3 S0-S8 verified | E0 and S6B-M Strict V4 are verified. Canonical X1 is remediation-required/non-credit after the frozen Docker-off time-domain gate failed; full-stack windows, Q0, calibration, its 78-run credit matrix, and Integrated V4 remain blocked/not run. |
| `EVM-299` | E0 Triton Environment And Profiler Baseline | Verified / Jira `SCRUM-210` Done | EVM-EPIC-26 | Source-local independent review passed at handoff `d8f83c79a2c516864663fc6c6957b2eed67aabba`. Three accepted runtime repetitions and all historical failures remain immutable. E0-AC-01..04 and purpose alignment passed; public review SHA `b2d13229...653ff2b9`, manifest SHA `29ecd65e...44800fa`, and private aggregates are unchanged. Claim is limited to one local physical node and one RTX 4080; CUPTI is not Triton kernel-overlap evidence. |
| `EVM-300` | S6B-M Triton Model Blue/Green | Verified / Acceptance Credit / Jira `SCRUM-211` Done | EVM-299 verified | Independent source-local audit and reviewer sign-off passed handoff `b5eb172dd30c75fc5831950a409bc8d2e4e7b926`. Strict V4 `49/49 + 28/28`, exact-1,000 continuity `24/24`, and the fresh `3 + 3 + 15` physical matrix recompute AC-01..04 as passed. Verified closure SHA `81980b36...34fcb62`; historical failed/zero-credit evidence remains immutable. |
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Non-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Historical suite `x1-canonical-20260830t075925z-7a121261`, RCA v2, and ledger through `s8-v4-0270` remain immutable. Remediation run `x1-clock-remediation-20260830T113944Z-34eec036` used the unchanged `180 s`/`100 ms` gate and failed its first Docker-off window: Windows discontinuities `0`, WSL `5`, backward `0`, unclassified gaps `0`, with four exact `34.8 s` intervals. No service event correlated with the steps, so guest time-service and Hyper-V implicit-sync settings were unchanged. `wsl --update` timed out after 180 seconds without output, `wsl --shutdown` passed, and official WSL ETW was not run because the session was non-admin. Public RCA `ca0bc72d...b66aa`; original private inventory `17` files / `5,820,112` bytes, aggregate `8548570b...a777a`, index `b5b98f58...37a4f`. A later audit found empty historical diagnostic schema `evm_x1_diag_ready` after the original readiness capture: business rows `0`, active sessions/locks `0/0`, only eight migration rows. Exact transactional removal and later residue-zero proof are append-only in correction `0d1cda1b...a0fc8`, private artifact/index `e0c0a644...51e9a` / `b53fdafe...509ee`, and ledger `s8-v4-0271`. Post-cleanup B0 actual-CUDA and GPU `1/1`, Prometheus `5/5`, queues `0/0/0`, and temporary schema/container/Kubernetes/file-SD/port/lease residue `0` pass. Decision remains `x1_clock_remediation_no_go`; full-stack windows, Q0, calibration, the 78-run matrix, and Integrated V4 were not started. |
