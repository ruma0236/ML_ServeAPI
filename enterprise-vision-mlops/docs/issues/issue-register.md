...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Eight pre-Q0 suites remain immutable zero-credit. Fresh suite `x1-canonical-20260829t215423z-8076e1f9` reached the exact four READY model/version identities but failed closed before Q0 because Triton omitted the empty optional repository-index `reason` member. A shared API/runner normalization, clean refreeze, and wholly fresh artifact/Q0/54-calibration suite remain required; 78-run credit remains not started. |
