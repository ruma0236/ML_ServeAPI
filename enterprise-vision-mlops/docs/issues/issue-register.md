...
| `EVM-301` | X1 Heterogeneous-model Concurrency | Remediation Required / Zero-Credit / Jira `SCRUM-212` In Progress | EVM-300 verified | Three pre-Q0 suites remain immutable zero-credit. The latest `x1-canonical-20260829t201707z-555cd04e` exposed traced/original parameter sharing that moved TinyMLP runtime weights to CPU before a CUDA oracle. Cleanup passed. Restore and fail-closed validate runtime CUDA residency, test/refreeze, then start a fresh artifact/Q0/calibration suite; 78-run credit remains not started. |
