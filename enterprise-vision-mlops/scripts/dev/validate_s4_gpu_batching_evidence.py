from __future__ import annotations

import argparse
import hashlib
import json
import subprocess
import sys
from pathlib import Path


ROOT = Path(__file__).resolve().parents[2]
SRC = ROOT / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

from evm.scale_validation.s4_evidence import (  # noqa: E402
    S4EvidenceValidationError,
    validate_s4_gpu_batching_closure,
    validate_s4_gpu_batching_evidence,
)
from evm.scale_validation.s4_runtime import S4RuntimeConfig  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate S4 GPU batching evidence.")
    parser.add_argument(
        "--evidence",
        type=Path,
        default=ROOT / "docs/status/evidence/s4-gpu-batching-experiment.json",
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/s4_gpu_batching_runtime.toml",
    )
    parser.add_argument(
        "--closure",
        type=Path,
        default=ROOT / "docs/status/evidence/s4-gpu-batching-closure.json",
    )
    parser.add_argument(
        "--data-root",
        type=Path,
        default=Path("F:/EnterpriseMLOps_Data/enterprise-vision-mlops"),
    )
    parser.add_argument("--private-root", type=Path)
    parser.add_argument("--git-revision")
    parser.add_argument("--require-closure", action="store_true")
    return parser.parse_args()


def load_bytes(path: Path, git_revision: str | None, git_root: Path) -> bytes:
    if not git_revision:
        return path.read_bytes()
    relative = path.resolve().relative_to(git_root).as_posix()
    return subprocess.run(
        ["git", "show", f"{git_revision}:{relative}"],
        cwd=git_root,
        check=True,
        capture_output=True,
    ).stdout


def main() -> int:
    args = parse_args()
    git_root = Path(
        subprocess.run(
            ["git", "rev-parse", "--show-toplevel"],
            cwd=ROOT,
            check=True,
            capture_output=True,
            text=True,
        ).stdout.strip()
    ).resolve()
    raw = load_bytes(args.evidence, args.git_revision, git_root)
    if b"\r\n" in raw or not raw.endswith(b"\n"):
        raise S4EvidenceValidationError("s4_evidence_not_canonical_lf")
    payload = json.loads(raw)
    config = S4RuntimeConfig.from_path(args.config, data_root=args.data_root)
    result = validate_s4_gpu_batching_evidence(
        payload,
        config=config,
        git_root=git_root,
        validation_revision=args.git_revision or "HEAD",
        private_root=args.private_root,
    )
    evidence_sha256 = hashlib.sha256(raw).hexdigest()
    result["evidence_sha256"] = evidence_sha256
    if args.require_closure:
        closure_raw = load_bytes(args.closure, args.git_revision, git_root)
        if b"\r\n" in closure_raw or not closure_raw.endswith(b"\n"):
            raise S4EvidenceValidationError("s4_closure_not_canonical_lf")
        result["closure"] = validate_s4_gpu_batching_closure(
            json.loads(closure_raw),
            experiment=payload,
            experiment_sha256=evidence_sha256,
            config=config,
            git_root=git_root,
            validation_revision=args.git_revision or "HEAD",
            private_root=args.private_root,
        )
        result["closure_sha256"] = hashlib.sha256(closure_raw).hexdigest()
    result["hash_source"] = args.git_revision or "worktree"
    print(json.dumps(result, sort_keys=True))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
