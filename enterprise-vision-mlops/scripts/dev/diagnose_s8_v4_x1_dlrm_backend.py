...
def build_triton_bundle(
    ...
    *,
    ...
    return triton_only_bundle(bundle, trace_enabled=trace_enabled)
...
def classify_diagnostic(
    ...
    if not direct_passed:
        return "direct_image_artifact_or_framework_failure"
    if not enabled_passed and disabled_passed:
        return "triton_trace_pipeline_correlated_stall"
    if not enabled_passed and not disabled_passed:
        return "triton_backend_or_model_instance_stall_independent_of_trace_setting"
...
def main() -> int:
    ...
    try:
        ...
        for mode, trace_enabled in (("trace-enabled", True), ("trace-disabled", False)):
            assert_scale_validation_gpu_lease_owner(
                run_id=lease.run_id,
                lease_id=lease.lease_id,
                fencing_token=lease.fencing_token,
        ...
        classification = classify_diagnostic(
            direct, mode_results["trace-enabled"], mode_results["trace-disabled"]
        )
        summary = {
            "schema_version": "evm.s8_v4.x1_dlrm_backend_diagnostic.v1",
            "diagnostic_id": diagnostic_id,
            "captured_at": utc_now(),
