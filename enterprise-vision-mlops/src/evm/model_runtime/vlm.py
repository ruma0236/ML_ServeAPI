...
def train_smolvlm_lora(config: SmolVlmTrainingConfig) -> dict[str, Any]:
    ...
    result = {
        ...
        "training_seconds": training_seconds,
        "metrics": metrics,
        "baseline_evaluation_uri": str(config.output_dir / "baseline-evaluation.json"),
        "evaluation_uri": str(config.output_dir / "adapted-evaluation.json"),
        "training_history_uri": str(config.output_dir / "training-history.json"),
    ...
    atomic_write_json(config.output_dir / "baseline-evaluation.json", baseline)
    atomic_write_json(config.output_dir / "adapted-evaluation.json", adapted)
    atomic_write_json(config.output_dir / "training-history.json", history)
    atomic_write_json(config.output_dir / "gpu-profile.json", {
        "runtime": runtime,
        "before": gpu_before,
        "after_training": result["gpu_after_training"],
