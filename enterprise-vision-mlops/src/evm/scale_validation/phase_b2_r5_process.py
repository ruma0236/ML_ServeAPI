"""Race-resistant process containment for the X1 Phase B2 r5 harness.
...
class TimeoutContract:
    """Timeouts shared with the r5 restore/fresh orchestrators."""
...
class _WindowsJobApi:
    ...
    def create_suspended_process(
        ...
        *,
        ...
        _storage, attributes, _jobs, _handles = self.initialise_attributes(
            job, (stdin_handle, stdout_handle, stderr_handle)
        )
        try:
            ...
            env_pairs = [f"{key}={value}" for key, value in environment.items()]
    ...
    def query_accounting(self, job: int) -> _JOB_BASIC_ACCOUNTING:
    ...
    def query_limit_flags(self, job: int) -> int:
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            failure_errors = [*errors, f"{type(exc).__name__}:{exc}"]
